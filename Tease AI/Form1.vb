...
retryStart:
			...
			FrmSettings.CBAutoDomPP.Checked = My.Settings.CBAutoDomPP
			...
			contextWMP.settings.volume = 100
			voiceDomWMP.Visible = False
			voiceDomWMP.uiMode = "none"
			voiceDomWMP.settings.volume = 100
...
FoundResponse:
		...
		If StrokeTauntTimer.Enabled = True Then
			...
			If FrmSettings.SliderSTF.Value = 5 Then ssh.StrokeTauntTick = ssh.randomizer.Next(10, 24)
		End If
...
CancelGoto:
	...
	Public Sub TypingDelay()
		...
		If FrmSettings.typeinstantlyCheckBox.Checked OrElse ssh.RapidCode OrElse ssh.RapidFire Then ssh.TypeDelay = 0
		If ssh.TypeDelay <> 0 Then
			ssh.TypeDelay /= 2
			If ssh.TypeDelay > 6 Then ssh.TypeDelay = 6
		End If
	...
	Public Sub TypingDelayGeneric()
		...
		ssh.TypeDelay = ssh.StringLength
		If FrmSettings.typeinstantlyCheckBox.Checked OrElse ssh.RapidCode OrElse ssh.RapidFire Then ssh.TypeDelay = 0
		If ssh.HypnoGen AndAlso CBHypnoGenNoText.Checked Then ssh.TypeDelay = 0
		If ssh.TypeDelay <> 0 Then
			ssh.TypeDelay /= 2.5
			If ssh.TypeDelay > 4 Then
				ssh.TypeDelay = 4
			End If
		End If
		Timer1.Start()
...
SkipIsTyping:
				...
				If FrmSettings.typeinstantlyCheckBox.Checked OrElse ssh.RapidCode OrElse ssh.RapidFire Then ssh.StringLength = 0
...
NullResponse:
				...
				If ssh.dommePresent AndAlso Not ssh.contactToUse Is Nothing Then
...
#End Region
	...
	Private Sub StrokeTauntTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles StrokeTauntTimer.Tick
		...
		If ssh.TempScriptCount = 0 Then
			...
			If FrmSettings.SliderSTF.Value = 5 Then ssh.StrokeTauntTick = ssh.randomizer.Next(12, 25)
		Else
...
#End Region
	...
	Private Sub SettingsButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnToggleSettings.Click
		If FrmSettings.Visible = True Then
			...
			ssh.StrokePace = ssh.StrokePaceMem
		Else
			FrmSettings.Visible = True
			ssh.StrokePaceMem = ssh.StrokePace
			ssh.StrokePace = 0
...
RinseLatherRepeat:
		...
		End If
		If StringClean.Contains("@ShowDomRandomImage") AndAlso My.Settings.DomImageDirRand <> "" Then
			Dim DirDomRandImage As New DirectoryInfo(My.Settings.DomImageDirRand)
			Dim DirDomRandomDir As New DirectoryInfo(DirDomRandImage.GetDirectories().ElementAt(ssh.randomizer.Next(DirDomRandImage.GetDirectories().Length)).FullName)
			If ssh.randomizer.Next(10) > 4 Then DirDomRandImage = DirDomRandomDir
		...
		If StringClean.Contains("#TagGarment") Then
			Dim tmpString5 As String = "garment"
			If ssh.DommeImageSTR <> "" Then
				Dim TagImageFolder5 As String = Directory.GetParent(ssh.DommeImageSTR).FullName
				If File.Exists(TagImageFolder5 + "\\ImageTags.txt") Then

					Dim TagCheckList5 As New List(Of String)()
					TagCheckList5 = Common.Txt2List(TagImageFolder5 + "\\ImageTags.txt")
					TagCheckList5 = StripBlankLines(TagCheckList5)

					For i16 As Integer = 0 To TagCheckList5.Count - 1

						If Not TagCheckList5(i16).Contains(Path.GetFileName(ssh.DommeImageSTR)) OrElse Not TagCheckList5(i16).Contains("TagGarment") Then
							Continue For
						End If
						If TagCheckList5(i16).Contains("TagGarmentCovering") Then

							TagCheckList5(i16) = TagCheckList5(i16).Replace("TagGarmentCovering", "")
						End If
						Dim TagSplit5() As String = Strings.Split(TagCheckList5(i16))

						For j6 As Integer = 0 To TagSplit5.Length - 1

							If (TagSplit5(j6).Contains("TagGarment")) Then

								tmpString5 = TagSplit5(j6).Replace("TagGarment", "")
								Exit For
							End If
				...
				End If
			End If
			StringClean = StringClean.Replace("#TagGarment", tmpString5)
		End If

		If StringClean.Contains("#TagUnderwear") Then
			Dim tmpString5 As String = "underwear"
			If ssh.DommeImageSTR <> "" Then
				Dim TagImageFolder5 As String = Directory.GetParent(ssh.DommeImageSTR).FullName
				If File.Exists(TagImageFolder5 + "\\ImageTags.txt") Then
					...
					For i16 As Integer = 0 To TagCheckList5.Count - 1
						...
						If Not TagCheckList5(i16).Contains(Path.GetFileName(ssh.DommeImageSTR)) OrElse Not TagCheckList5(i16).Contains("TagUnderwear") Then
							Continue For
						End If
						...
						For j6 As Integer = 0 To TagSplit5.Length - 1
							...
							If (TagSplit5(j6).Contains("TagUnderwear")) Then

								tmpString5 = TagSplit5(j6).Replace("TagUnderwear", "")
								Exit For
							End If
				...
				End If
			End If
			StringClean = StringClean.Replace("#TagUnderwear", tmpString5)
		End If

		If StringClean.Contains("#TagTatoo") Then
			Dim tmpString5 As String = "tatoo"
			If ssh.DommeImageSTR <> "" Then
				Dim TagImageFolder5 As String = Directory.GetParent(ssh.DommeImageSTR).FullName
				If File.Exists(TagImageFolder5 + "\\ImageTags.txt") Then
					...
					For i16 As Integer = 0 To TagCheckList5.Count - 1
						...
						If Not TagCheckList5(i16).Contains(Path.GetFileName(ssh.DommeImageSTR)) OrElse Not TagCheckList5(i16).Contains("TagTatoo") Then
							Continue For
						End If
						...
						For j6 As Integer = 0 To TagSplit5.Length - 1
							...
							If (TagSplit5(j6).Contains("TagTatoo")) Then

								tmpString5 = TagSplit5(j6).Replace("TagTatoo", "")
								Exit For
							End If
				...
				End If
			End If
			StringClean = StringClean.Replace("#TagTatoo", tmpString5)
		End If
		If StringClean.Contains("#TagSexToy") Then
			Dim tmpString5 As String = "toy"
			If ssh.DommeImageSTR <> "" Then
				Dim TagImageFolder5 As String = Directory.GetParent(ssh.DommeImageSTR).FullName
				If File.Exists(TagImageFolder5 + "\\ImageTags.txt") Then
					...
					For i16 As Integer = 0 To TagCheckList5.Count - 1
						...
						If Not TagCheckList5(i16).Contains(Path.GetFileName(ssh.DommeImageSTR)) OrElse Not TagCheckList5(i16).Contains("TagSexToy") Then
							Continue For
						End If
						...
						For j6 As Integer = 0 To TagSplit5.Length - 1
							...
							If (TagSplit5(j6).Contains("TagSexToy")) Then

								tmpString5 = TagSplit5(j6).Replace("TagSexToy", "")
								Exit For
							End If
				...
				End If
			End If
			StringClean = StringClean.Replace("#TagSexToy", tmpString5)
		End If
		If StringClean.Contains("#TagFurniture") Then
			Dim tmpString5 As String = "furniture"
			If ssh.DommeImageSTR <> "" Then
				Dim TagImageFolder5 As String = Directory.GetParent(ssh.DommeImageSTR).FullName
				If File.Exists(TagImageFolder5 + "\\ImageTags.txt") Then
					...
					For i16 As Integer = 0 To TagCheckList5.Count - 1
						...
						If Not TagCheckList5(i16).Contains(Path.GetFileName(ssh.DommeImageSTR)) OrElse Not TagCheckList5(i16).Contains("TagFurniture") Then
							Continue For
						End If
						...
						For j6 As Integer = 0 To TagSplit5.Length - 1
							...
							If (TagSplit5(j6).Contains("TagFurniture")) Then

								tmpString5 = TagSplit5(j6).Replace("TagFurniture", "")
								Exit For
							End If
				...
				End If
			End If
			StringClean = StringClean.Replace("#TagFurniture", tmpString5)
		End If
		If StringClean.Contains("@ImageTag(") Then
			Dim TagFlag As String = GetParentheses(StringClean, "@ImageTag(")
			Dim i As Integer = 0
			While i < 10
				If ShowImage(GetLocalImage(TagFlag), False) Then
					Exit While
				End If
				i += 1
			End While
		...
		If StringClean.Contains("@ImageTagOr(") Then
			...
			Dim i As Integer = 0
			While i < 10
				If ShowImage(GetLocalImage(TagFlag,, "Or"), False) Then
					Exit While
				End If
				i += 1
			End While
		...
		If StringClean.Contains("@ImageTagAny(") Then
			...
			Dim i As Integer = 0
			While i < 10
				If ShowImage(GetLocalImage(TagFlag,, "Any"), False) Then
					Exit While
				End If
				i += 1
			End While
		...
		If StringClean.Contains("@ImageTagFirst(") Then
			...
			Dim i As Integer = 0
			While i < 10
				If ShowImage(GetLocalImage(TagFlag,, "First"), False) Then
					Exit While
				End If
				i += 1
			End While
		...
		If StringClean.Contains("@ShowLikedImage") Then
			Dim i As Integer = 0
			While i < 10
				If ShowImage(GetImageData(ImageGenre.Liked).Random(), False) Then
					Exit While
				End If
				i += 1
			End While
		...
		If StringClean.Contains("@ShowDislikedImage") Then
			Dim i As Integer = 0
			While i < 10
				If ShowImage(GetImageData(ImageGenre.Disliked).Random(), False) Then
					Exit While
				End If
				i += 1
			End While
		...
		If StringClean.Contains("@ShowBlogImage") Then
			Dim i As Integer = 0
			While i < 10
				If ShowImage(GetImageData(ImageGenre.Blog).Random(), False) Then
					Exit While
				End If
				i += 1
			End While
		...
		If StringClean.Contains("@ShowLocalImage(") Then
			...
			Dim ix As Integer = 0
			While ix < 10
				If ShowImage(tmpImgToShow, False) Then
					Exit While
				End If
				ix += 1
			End While
		...
		If StringClean.Contains("@Slideshow(") Then
			...
			CustomSlideshowTimer.Interval = 3000
			If LCase(SlideFlag).Contains("slow") Then CustomSlideshowTimer.Interval = 6000
			If LCase(SlideFlag).Contains("very slow") Then CustomSlideshowTimer.Interval = 9000
			If LCase(SlideFlag).Contains("fast") Then CustomSlideshowTimer.Interval = 1200
...
OrgasmDecided:
		...
		If StringClean.Contains("@PlayAudioNoWait[") Then
			...
			While canPlay AndAlso contextWMP.playState <> WMPPlayState.wmppsPlaying
				If Not sw.IsRunning Then sw.Start()
				Application.DoEvents() ' dubious infinite loop
		...
		If StringClean.Contains("@StopAudio") Then
			If contextWMP.playState = WMPPlayState.wmppsPlaying OrElse contextWMP.playState = WMPPlayState.wmppsPaused Then
				contextWMP.Ctlcontrols.stop()
				contextWMP.currentPlaylist.clear()
			End If
			If voiceDomWMP.playState = WMPPlayState.wmppsPlaying OrElse voiceDomWMP.playState = WMPPlayState.wmppsPaused Then
		...
		If StringClean.Contains("@JumpVideoUp") Then
			If StringClean.Contains("@JumpVideoUp(") Then
				...
				If (jumpFlag <> 0) Then
					ssh.JumpPercentVid = Convert.ToInt32(jumpFlag)
				Else
					ssh.JumpPercentVid = 0
				End If
				StringClean = StringClean.Replace("@JumpVideoUp(" & jumpFlag & ")", "")
			Else
				StringClean = StringClean.Replace("@JumpVideoUp", "")
			End If
			Jump2Random(Nothing, Nothing, Up:=True)
		End If
		If StringClean.Contains("@JumpVideoDown") Then
			If StringClean.Contains("@JumpVideoDown(") Then
				...
				Else
					ssh.JumpPercentVid = 0
				End If
				StringClean = StringClean.Replace("@JumpVideoDown(" & jumpFlag & ")", "")
			Else
				StringClean = StringClean.Replace("@JumpVideoDown", "")
			End If
			Jump2Random(Nothing, Nothing, Down:=True)
		End If
		If StringClean.Contains("@JumpVideo") Then
			If StringClean.Contains("@JumpVideo(") Then
				...
				Else
					ssh.JumpPercentVid = 0
				End If
				StringClean = StringClean.Replace("@JumpVideo(" & jumpFlag & ")", "")
			Else
				StringClean = StringClean.Replace("@JumpVideo", "")
			End If
			Jump2Random(Nothing, Nothing)
		End If
		If StringClean.Contains("@JumpAudioUp") Then
			If StringClean.Contains("@JumpAudioUp(") Then
				Dim jumpFlag As String = GetParentheses(StringClean, "@JumpAudioUp(")
				If (jumpFlag <> 0) Then
					ssh.JumpPercentAudio = Convert.ToInt32(jumpFlag)
				Else
					ssh.JumpPercentAudio = 0
				End If
				StringClean = StringClean.Replace("@JumpAudioUp(" & jumpFlag & ")", "")
			Else
				StringClean = StringClean.Replace("@JumpAudioUp", "")
			End If
			Jump2Random(Nothing, Nothing, Up:=True, Audio:=True)
		End If
		If StringClean.Contains("@JumpAudioDown") Then
			If StringClean.Contains("@JumpAudioDown(") Then
				Dim jumpFlag As String = GetParentheses(StringClean, "@JumpAudioDown(")
				If (jumpFlag <> 0) Then
					ssh.JumpPercentAudio = Convert.ToInt32(jumpFlag)
				Else
					ssh.JumpPercentAudio = 0
				End If
				StringClean = StringClean.Replace("@JumpAudioDown(" & jumpFlag & ")", "")
			Else
				StringClean = StringClean.Replace("@JumpAudioDown", "")
			End If
			Jump2Random(Nothing, Nothing, Down:=True, Audio:=True)
		End If
		If StringClean.Contains("@JumpAudio") Then
			If StringClean.Contains("@JumpAudio(") Then
				Dim jumpFlag As String = GetParentheses(StringClean, "@JumpAudio(")
				If (jumpFlag <> 0) Then
					ssh.JumpPercentAudio = Convert.ToInt32(jumpFlag)
				Else
					ssh.JumpPercentAudio = 0
				End If
				StringClean = StringClean.Replace("@JumpAudio(" & jumpFlag & ")", "")
			Else
				StringClean = StringClean.Replace("@JumpAudio", "")
			End If
			Jump2Random(Nothing, Nothing, Audio:=True)
		End If
		...
		End If
		If StringClean.Contains("@SetRandom") Then
			ssh.glitterDommeNumber = 4
			LoadDommeImageFolder()
			StringClean = StringClean.Replace("@SetRandom", "")
		End If
	...
	Public Function GetLocalImage(LocTag As String, Optional isDommeTag As Boolean = False, Optional selectionType As String = "All") As String
		...
		If File.Exists(fileToCheck) Then
			...
			Select Case selectionType
				...
				Case "First"
					For i As Integer = 0 To TagList.Count - 1
						For n As Integer = 1 To LocTagArray.Count - 1
							If Not TagList(i).Contains(LocTagArray(n)) OrElse Not TagList(i).Contains(LocTagArray(0)) Then
								TaggedList.Add(TagList(i))
								Exit For
							End If
						Next
...
#End If
	...
	Public Function GetFilter(ByVal FilterString As String, Optional ByVal Linear As Boolean = False) As Boolean
		...
		Try
			If Linear = False Then
				If FilterString.Includes("@DommeTag(") Then
					If ssh.LockImage OrElse Not mainPictureBox.Visible OrElse ssh.CustomSlideEnabled Then
						Return False
					Else
				...
				If FilterString.Contains("@ImageTagAny(") Then
					...
					End If
				End If
				If FilterString.Contains("@ShowDomRandomImage") AndAlso (ssh.LockImage OrElse Not mainPictureBox.Visible OrElse ssh.CustomSlideEnabled) Then
					Return False
				End If
				If FilterString.Contains("@ShowImage") AndAlso (ssh.LockImage OrElse Not mainPictureBox.Visible OrElse ssh.CustomSlideEnabled) Then
					Return False
				End If
				' ################## @Show-Category-Image #####################
	  ...
	  My.Settings.CBIGay = False And My.Settings.CBIMaledom = False And My.Settings.CBICaptions = False And My.Settings.CBIGeneral = False Then Return False
			...
			ElseIf ssh.LockImage OrElse ssh.CustomSlideEnabled Then
				If Not FilterString.Contains("@PlayVideoNoWait") AndAlso FilterString.Contains("@PlayVideo") Then
					Return False
				End If
				If FilterString.Contains("@PlayRedLightGreenLight") OrElse FilterString.Contains("@PlayAvoidTheEdge") Then
					Return False
				End If

			End If
			If Not FilterString.Contains("@PlayVideoNoWait") AndAlso (FilterString.Contains("@PlayVideo") OrElse FilterString.Contains("@PlayRedLightGreenLight") OrElse
				FilterString.Contains("@PlayAvoidTheEdge")) AndAlso Not FilterString.Contains("@StopVideo") AndAlso ((DomWMP.playState = WMPPlayState.wmppsPlaying) OrElse (DomWMP.playState = WMPPlayState.wmppsPaused)) Then
				Return False
			End If
			'▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
			...
			If FilterString.Contains("@Contact3") Then
				If ssh.GlitterTease = False Or Not ssh.Group.Contains("3") Then Return False
			End If
			If FilterString.Contains("@Contact(") Then

				If Not ssh.GlitterTease Then
					Return False
				End If

				Dim getContacts As String = GetParentheses(FilterString, "@Contact(")
				getContacts = FixCommas(getContacts)
				Dim array5() As String = getContacts.Split({","}, StringSplitOptions.RemoveEmptyEntries)
				For Each contact As String In array5
...
#Region "---------------------------------------------------- Domme-WMP -------------------------------------------------------"
	Private Sub contextWMP_PlayStateChange(sender As Object, e As _WMPOCXEvents_PlayStateChangeEvent) Handles contextWMP.PlayStateChange
		If contextWMP.playState = WMPPlayState.wmppsStopped Then

			contextWMP.currentPlaylist.clear()
		End If
	End Sub

	Private Sub voiceDomWMP_PlayStateChange(sender As Object, e As _WMPOCXEvents_PlayStateChangeEvent) Handles voiceDomWMP.PlayStateChange
		If voiceDomWMP.playState = WMPPlayState.wmppsStopped Then
...
#End Region  ' PictureStrip
	Public Function LoadRandomFolder(ByVal baseDirectory As String) As String
	...
	Public Sub InitDommeImageFolder()
		If My.Settings.CBAutoDomPP OrElse FrmSettings.CBRandomDomme.Checked Then
			...
			If String.IsNullOrEmpty(avatarImage) Then
			...
			End If
			If Not String.IsNullOrEmpty(glitterImage2) Then
				FrmSettings.GlitterAV.Image = Image.FromFile(glitterImage2)
				If Not FrmSettings.CBRandomDomme.Checked Then
					My.Settings.GlitterAV = glitterImage2
				End If
			End If
		End If
		CheckRandomOpportunities()
		If FrmSettings.CBRandomGlitter.Checked AndAlso FrmSettings.CBRandomGlitter.Enabled Then
			...
			While Contact3dir.Name = ssh.tempDomName OrElse Contact3dir.Name = Contact2dir.Name OrElse Contact3dir.Name = Contact1dir.Name
				Contact3path = LoadRandomFolder(My.Settings.RandomImageDir)
			...
			End If
			If glitterImage <> "" Then
				My.Settings.GlitterAV2 = glitterImage
			...
			End If
			If glitterImage <> "" Then
				My.Settings.GlitterAV3 = glitterImage
		...
		If (Directory.Exists(RandImgDirectoryD)) Then
			...
			' Conversions.ToString(My.Settings.PropertyValues["DomImageDirRand"].Property.DefaultValue)
			My.Settings.DomImageDirRand = RandImgPathDirectoryD

		End If
		If (Directory.Exists(ClipPathDirectoryD)) Then

			Dim def As String = My.Settings.PropertyValues("VideoSoftcoreD").Property.DefaultValue
			...
			Else
				...
				My.Settings.VideoSoftcoreD = def
			End If
			def = My.Settings.PropertyValues("VideoHardcoreD").Property.DefaultValue
			...
			Else
				...
				My.Settings.VideoHardcoreD = def
			End If
			def = My.Settings.PropertyValues("VideoLesbianD").Property.DefaultValue
			...
			Else
				...
				My.Settings.VideoLesbianD = def
			End If
			def = My.Settings.PropertyValues("VideoBlowjobD").Property.DefaultValue
			...
			Else
				...
				My.Settings.VideoBlowjobD = def
			End If
			def = My.Settings.PropertyValues("VideoFemsubD").Property.DefaultValue
			...
			Else
				...
				My.Settings.VideoFemsubD = def
			End If
			def = My.Settings.PropertyValues("VideoJOID").Property.DefaultValue
			...
			Else
				...
				My.Settings.VideoJOID = def
			End If
			def = My.Settings.PropertyValues("VideoCHD").Property.DefaultValue
			...
			Else
				...
				My.Settings.VideoCHD = def
			End If
			def = My.Settings.PropertyValues("VideoGeneralD").Property.DefaultValue
	...
	Public Sub LoadDommeImageFolder(Optional ByVal newSet As Boolean = True)
		'check which domme should be loaded
		If Not ssh.newSlideshow Then
			If ssh.glitterDommeNumber = 0 Then
				If Not FrmSettings.CBRandomDomme.Checked Then
					If ssh.dommePresent AndAlso Not ssh.SlideshowMasterDomme Is Nothing Then
			...
			' ssh.contactToUse = ssh.SlideshowMain
		End If
		If newSet OrElse ssh.SlideshowMain.TypeName = "" Then
	...
	Public Sub StrokeSpeedCheck()
		...
		If ssh.StrokeFaster = True Then
			If ssh.SubStroking = True And ssh.SubEdging = False And ssh.SubHoldingEdge = False Then
				...
				Dim Stroke123 As Integer = ssh.randomizer.Next(3, 7)
...
#End Region  ' Apps
	...
	Public Sub Edge()
		...
		EdgeCountTimer.Start()
		ssh.SubEdging = True
		ssh.EdgeTauntInt = ssh.randomizer.Next(2, 16)
	...
	Private Function updateDommeName(ByVal stringToCheck As String)
		...
		ElseIf stringToCheck.Contains("@Contact3") Then
			...
			ssh.contactToUse = ssh.SlideshowContact3
		ElseIf stringToCheck.Contains("@RandomContact") OrElse stringToCheck.Contains("@Contact(") Then

			Dim casual As Integer = 0
			Dim ContactsListToPick As New List(Of String)
			...
			ElseIf (stringToCheck.Contains("@Contact(")) Then
				...
				Dim getContacts As String = GetParentheses(stringToCheck, "@Contact(")
				getContacts = FixCommas(getContacts)
				Dim array() As String = getContacts.Split({","}, StringSplitOptions.RemoveEmptyEntries)
				For Each contact As String In array
				...
				Next
			End If
			casual = ssh.randomizer.Next(0, ContactsListToPick.Count)
	...
	Private Sub selectVideo(ByVal StringClean As String)
		...
		If VideoClean.Contains("*") OrElse File.Exists(VideoClean) Then
			...
			If VideoClean.Contains("*") Then
				...
				Catch ex As Exception
					MessageBox.Show(Me, "Error  matching " & Path.GetFileName(VideoClean) & " in " & Path.GetDirectoryName(VideoClean) & "!" & Environment.NewLine & Environment.NewLine &
				 "Please make sure that valid files exist and that the wildcards are applied correctly in the script." & ex.ToString(), "Error!", MessageBoxButtons.OK, MessageBoxIcon.Hand)

				End Try
				DomWMP.Visible = True
				mainPictureBox.Visible = False
			ElseIf File.Exists(VideoClean) Then
				DomWMP.URL = VideoClean
				DomWMP.Visible = True
				mainPictureBox.Visible = False
			End If
		Else
			MessageBox.Show(Me, "Video file " & Path.GetFileName(VideoClean) & " was not found in " & Path.GetDirectoryName(VideoClean) & "!" & Environment.NewLine & Environment.NewLine &
				 "Please make sure that valid files exist and that the wildcards are applied correctly in the script.", "Error!", MessageBoxButtons.OK, MessageBoxIcon.Hand)
		End If
		DomWMP.stretchToFit = True
	End Sub
